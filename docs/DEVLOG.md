...
- 2026-09-08：对齐 Java MCP 工具契约的两处关键差异（`McpToolResults.error` + `McpToolAnnotations` + `requireReadOnlyHint`）：①业务错误置 `isError=true`——工具的参数校验失败、上游查询失败此前以纯文本返回（isError 缺省 false），主服务消费端无法把这类错误从事实数据中分离；现在 `errorContent` 统一带错误标记，server 据此把 `tools/call` 结果置 `isError: true`（Execute 返回非 nil error 的路径同步带标记）。②工具 annotations——`tools/list` 现在下发 `annotations.readOnlyHint`（调用方按此决定要不要拦下让用户确认），`NewServer` 启动校验未声明 readOnlyHint 的工具直接拒绝注册（对齐 Java requireReadOnlyHint「工具必须自报是读还是写」），现有 8 个工具全部声明只读。
- 2026-09-08：补齐前端直接消费的四个 API 契约（对齐 Java `SampleQuestionController`/`DashboardServiceImpl`/`DashboardPerformanceVO`）：新增 `GET /sample-questions/random` 端点（前端欢迎页调用，此前 404），支持 `limit` 参数缺省 3、上限 20（防调用方要大数把整表捞走），字面量段优先于 `/:id` 模板匹配；Dashboard Overview 补 `activeSessions` KPI（窗口内有消息的会话数，按 conversation_id+user_id 组合去重，拼接法兼容 SQLite/PostgreSQL，与上一窗口对比给 delta/deltaPct）；Dashboard Overview 与 Performance 响应补 `engine: "workflow"` 常量字段（前端按 engine 区分渲染）；Performance 的样本数字段名从 `totalTraces` 改为 `sampleCount`（与 Java VO 及前端消费一致，语义为区分「没有采样」和真实的 0% 成功率）。
- 2026-09-08：【安全】流式任务停止接口补属主校验（对齐 Java `StreamTaskManager.cancelByUser`）：`taskId` 是时间有序可预测的雪花 ID，不是访问凭证，此前 `POST /rag/v3/stop` 传入任意 taskId 即可取消他人任务。现在 `register` 时把属主用户 ID 写入 Redis `ragent:stream:owner:{taskId}`（属主先于收尾回调落地，属主进 Redis 而非只留本地——停止请求可能落在没跑这条流的节点上）；`cancelByUser` 比对属主，不匹配直接拒绝（错误文案「任务不存在或已结束」，不区分「不存在」与「非属主」，免得停止接口变成他人任务的探测器）；取消广播载荷升级为 `taskId|requester`，执行端收到广播与预埋标记时复核发起方（系统侧回收 `__system__` 无条件放行，用户侧只认精确属主，属主未落地时一律不认），非属主埋下的取消标记在 register 那一刻被拦截且不置 cancelled（避免短路复核）。系统侧回收（超时清理）走 `cancel` 保持无条件放行。StopTask 服务签名改为 `(taskID, requester) error`。
- 2026-09-08：对齐 Java `KnowledgeDocumentScheduleJob` + `knowledgeChunkExecutor` 的后台调度执行模型：到期文档刷新从批内串行改为并行——worker 池取 CPU 核数、任务队列容量 200（对齐 Java 线程池队列 + AbortPolicy），队列满被拒时释放调度锁留给下一轮扫描（对齐 Java RejectedExecutionException 释放锁），单个慢文档不再推迟同批其余文档与卡死恢复 ticker 的触发；卡死恢复 ticker 补 30 秒首轮延迟（对齐 Java `fixedDelay=60s, initialDelay=30s`），进程崩溃重启后卡死文档最多多等 30 秒而不是整一分钟；同时修复主服务对 `DocumentScheduleService.Run` 的双重启动（同一服务曾以两个 goroutine 运行，重复扫描空转）。并行化后调度服务的 fileStore/chunkStarter 采集点需要并发安全，测试以 `-race` 验证。
- 2026-09-08：补齐检索归因日志族（对齐 Java `RerankPostProcessor.logAttribution` + `ChannelAttribution`）：融合处理器现在按 chunk key 从通道结果反查每条证据的来源通道，并在 metadata 写入 `retrieval_channel` 标记（多路命中取首个命中通道），Rerank 前后按通道统计候选分布并打印「检索归因 - Rerank 输入按通道」；图谱通道在场时额外打印「检索归因 - 图谱证据存活 survived/recalled」——图谱大量进入 Rerank 却几乎不存活说明其为纯成本，应下调 fusion 权重或优化长证据可排性。通道标签中文化（向量/关键词/图谱/联网），多通道命中在统计口径上各计一次（metadata 标记为单值，与 Java 集合归因的细微差异源于 Go 融合去重按首通道定归属）。
- 2026-09-08：对齐 Java `WebSearchChannel` 的联网检索细节：单次检索数量 `count` 补上限 20（超过截断，非法回退默认 5）；API Key 解析改为「优先配置 `api-key`，为空回退环境变量 `YDC_API_KEY`」（新增 `rag.ResolveWebSearchAPIKey`，主服务装配时解析，启用条件因此可在仅设环境变量时成立）；联网证据文本编排对齐 `toChunk`——标题带【】、URL 带「来源: 」前缀，替换原先的裸字段 join，保证下游拼接 Prompt 时引用信息不丢失。
- 2026-09-08：对齐 Java `SearchChannelProperties.afterPropertiesSet` 的检索预算与作用域启动校验（配置矛盾启动即失败，胜过线上悄悄少召回或死代码分支）：`rag.search.fusion.rerank-candidate-limit < default-top-k` 违反漏斗单调不变式（送入 Rerank 的候选池不得小于最终条数）报错；`intent-directed.min-intent-score < INTENT_MIN_SCORE(0.35)` 报错（该配置不会产生任何效果）；`vector-global.confidence-threshold` 必须落在 `(min-intent-score, 1]` 内，否则「低置信退化为全局」或「高置信收窄」其中一条作用域分支连同补充路变成死代码；`supplement-ratio >= 1` 或 NaN 报错（取 1 等于把高置信命中库名额清零）。Java 的 `recall-budget`（每通道统一召回扇出基数，默认 20）为 Go 尚缺的配置项，涉及各通道取数深度的行为统一，暂不在本轮校验对齐中引入。
- 2026-09-08：对齐 Java `MilvusVectorRetrieverService.searchShared` 的检索参数：Milvus 向量检索现在显式下发 `metric_type`（取自配置的度量类型）与 `ef=128`（HNSW 搜索深度，不传走索引默认值），与 Java searchParams 一致。已知架构差异暂不对齐：Java 是共享物理 collection + `collection_name` 标量过滤（含 collection_name 倒排索引、HNSW M=48/efConstruction=200 建索引参数），Go 保持每知识库一个 collection 的 schema，两者检索语义等价但物理布局不同，同步需迁移存量数据，超出最小修改范围。
- 2026-09-08：对齐 Java `RetrievalEngine.retrieve` 的子问题级并行检索：多子问题意图原先串行逐个检索且任一失败即整体中断，现改为每个子问题独立 goroutine 并发执行（按索引回填保持子问题顺序，合并结果稳定），单个子问题检索失败时记 error 日志并降级为空结果，不再阻断其余子问题——与 Java `ragContextExecutor` 并行 + 异常降级为空 `SubQuestionContext` 的语义一致。
- 2026-09-08：对齐 Java 16984b95 的证据相关性闸门（对齐 `EvidenceGatePostProcessor`）：检索只保证返回最像的 N 条，库里没答案时照样满额返回，噪声必然进提示词——现于 Rerank 后按「整批最高精排分」判定证据批级去留，低于 `rag.search.evidence.min-rerank-score`（默认 0.2，<=0 关闭，>1 或 NaN 启动报错）整批丢弃，过线后弱证据一并保留。精排出分契约对齐 `BaiLianRerankClient`：同一个精排分写两处（`score` 会被下游覆写、`rerankScore` 留给闸门），`RetrievedChunk`/`rerank.Chunk` 新增 `RerankScore *float64`（nil 表示该条没跑过精排），HTTP 精排客户端对未出分条目压 0 沉底且不写 `rerankScore`，API 返回不足 topN 时按原顺序回填未打分候选；noop 降级只截断不打分，闸门遇到全批无分 warn 后放行（照拦等于在精排最不稳时关掉整条 KB 侧）。闸门开着却关了精排（`rag.rerank.enabled=false`）启动即报错而不是上线后恒放行。同时补齐「检索归因 - 精排分布」日志（打本批精排分高低两端，用于校准闸门下限）。Go 侧闸门落在 `RerankRetriever` 精排之后、metadata 富化之前，与 Java 后置处理器顺序（Rerank 10 → EvidenceGate 15 → MetadataEnrichment 20）同构；Go 特有的强关键词锚点回捞保留，发生在闸门判定之前。
- 2026-09-08：对齐 Java a6fb7139 的 embedding 链路：客户端新增批量分片（`MaxBatchSize`，百炼 10、SiliconFlow/AIHubMix 32，0 不限），`EmbedBatch` 超限自动分片按序回填；`dimensions` 参数从仅 ollama 发送改为所有 OpenAI 兼容提供商发送（Matryoshka 系模型依赖该参数指定输出维度）；非 ollama 提供商补充 `encoding_format: float`；ollama 免鉴权（`RequiresAPIKey=false`）。配置对齐：bailian 补 `embedding: /compatible-mode/v1/embeddings` endpoint，ollama embedding 端点改回 OpenAI 兼容的 `/v1/embeddings`（与响应解析一致），默认候选切换 `qwen-emb-8b = bailian/text-embedding-v4` 并补 aihubmix provider 与 `text-embedding-3-large` 候选。文档分块任务与重建分块路径从逐条 RPC 改为批量向量化；摄取管道与文档任务补齐「向量条数必须与分块数一致」强校验，替换原先越界静默跳过。
- 2026-09-08：同步行内引用规则提示词为 Java `answer-citation-rules.st` 完整版（41 行）：补齐输入标记豁免声明（编号标注仅供模型读取、进入回答的唯一形式是角标）、角标紧邻标点、禁止汇总标注、标题不加引用、多资料连写、混合场景数字引用只用于知识资料、正误对照示例等；此前 Go 侧为 9 行简化版。
- 2026-09-08：补齐意图回答规则与 MCP 模板在提示词组装侧的消费（对齐 Java DefaultContextFormatter/RAGPromptService）：KB 上下文按证据归属意图注入 `promptSnippet` 回答规则（单一归属意图 `<rules>` 直注、多意图去重后编号合并，前置于文档块）；MCP 工具结果段按 `toolId → 意图` 映射为每个工具注入其 `promptSnippet` 规则段（对齐 mcp-intent-rules/mcp-section 模板语义）；`PromptContext` 新增 `McpIntents`，MCP-only 场景恰好命中一个 MCP 意图时使用该意图的 `promptTemplate` 作为基础模板（对齐 planMcpOnly，此前 MCP 意图模板无任何消费路径）。
- 2026-09-08：对齐 Java 6e1a55ab 的 enable_thinking 提供商感知：`OpenAICompatibleChatClient` 新增 `SupportsEnableThinkingParam` 声明（默认 false），未声明认识该字段的提供商不再无条件发送 `enable_thinking`（该字段是 DashScope 系私有扩展，发给不认识它的网关会被判为 unknown_parameter 直接 400）；主服务按 provider 名（bailian/siliconflow/dashscope）声明支持，与 Java BaiLianChatClient/SiliconFlowChatClient 覆写语义一致。
- 2026-09-08：对齐 Java 回答请求参数与 systemOnly 判定：主回答请求设置 `temperature = hasMcp ? 0.3 : 0`、`topP = hasMcp ? 0.8 : 1`，`DefaultPromptBuilder.Build` 不再硬编码 `maxTokens=1024`（对齐 Java 不设上限，避免长回答截断）；systemOnly 判定改为严格单个 SYSTEM 意图（一个子问题命中多个 SYSTEM 意图时继续走检索，对齐 `IntentResolver.isSystemOnly`），customPrompt 取所有命中节点第一个非空 promptTemplate（不限 SYSTEM kind，对齐 handleSystemOnly 的 flatMap）；systemOnly 请求改为 system + history + user 三段消息、temperature 0.7、不再注入检索上下文（对齐 `streamSystemResponse`）。
- 2026-09-08：同步 Java 意图节点 `require_confirm` 字段全链路（对齐 765d2b80/260830 升级脚本）：`t_intent_node` 新增 `require_confirm SMALLINT NOT NULL DEFAULT 0` 列与迁移脚本；model/DTO（创建、更新、树形响应）与 `rag.IntentNode` 视图（布尔形态）均补齐字段；创建与更新走 `normalizeRequireConfirm`（非 MCP 节点一律落 0，确认标志只在工具调用链路上被读取）；更新接口补齐 `mcpToolId` 修改能力，节点改成非 MCP 后 `mcpToolId/requireConfirm` 同步清空，避免改回 MCP 时残留标志突然生效，对齐 Java `IntentTreeServiceImpl.updateNode`。
- 2026-09-08：对齐意图分类阈值与 Java RAGConstant：意图准入 `IntentMinScore=0.35`、候选总量 `MaxIntentCount=3` 作为固定常量，`NewIntentResolver` 未显式传参时默认使用；主服务不再借用检索通道的 `min-intent-score`（0.4，仅作用于检索作用域判定）作为分类阈值，0.35~0.4 之间的意图不再被整体丢弃，意图总数上限从 5 收敛为 3。
...
- 2026-08-23：补齐 XLSX 规整表格规范化；原生解析器按单元格坐标保留稀疏列，支持 `headerRows` 多行表头展平、横向/纵向合并单元格展开、全空列与全空数据行过滤，对齐 Java 快速解析档的表格语义。
