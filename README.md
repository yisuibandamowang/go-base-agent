...
初始化器会读取数据集目录下的 `initializer.properties`，支持 `${ENV}` 和 `${ENV:default}` 环境变量占位符；其中的 `application.config` 用于指定主服务配置文件，服务地址、管理员账号、确认词、清理锁租约和预热/文档超时可作为默认值。命令行参数优先级高于 properties，仍可通过 `--initializer-config` 显式指定配置文件。
