...
func newTicketQueryTool() *Tool {
	return &Tool{
		...
		Properties: map[string]propDesc{
			...
			"status":       {Type: "string", Description: "工单状态筛选：待处理、处理中、已解决、已关闭，不填则查询全部状态", Enum: []string{"待处理", "处理中", "已解决", "已关闭"}},
			"priority":     {Type: "string", Description: "优先级筛选：紧急、高、中、低，不填则查询全部优先级", Enum: []string{"紧急", "高", "中", "低"}},
			...
			"queryType":    {Type: "string", Description: "查询类型：summary(汇总概览)、list(工单列表)、stats(统计分析)", Enum: []string{"summary", "list", "stats"}, Default: "summary"},
