...
func TestUploadInternalURLDocumentCreatesAndChunksAllDocs(t *testing.T) {
	...
	if resp.Data.Documents[0].Status != "success" || resp.Data.Documents[0].ChunkCount == 0 {
		t.Fatalf("expected chunked document, got %s", w.Body.String())
	}
	var count int64
